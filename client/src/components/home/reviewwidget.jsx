...
export default function ReviewWidget() {
    ...
    useEffect(() => {
        fetch(`${baseUrl}/getreviews`)
            .then((backendresponse) => backendresponse.json())
            // set the googlereviews array in reviewobj to the response data
            .then((reviews) => {
                console.log("review data:", reviews);
                // yelpreviews have reviews.yelp === true
                const yelpdata = reviews.filter((review) => review.yelp === true);
                const googledata = reviews.filter((review) => review.yelp === false);

                let starSum = [...reviews].reduce((accumulator, currentValue) => {
                ...
                setReviewObj({
                    ...
                    yelpreviewaverage:
                        Math.round(
                            (yelpdata.reduce((accumulator, currentValue) => accumulator + currentValue.rating, 0) /
                                yelpdata.length) *
                                10
                        ) / 10,
                    ...
                    googlereviewavg:
                        Math.round(
                            (googledata.reduce((accumulator, currentValue) => accumulator + currentValue.stars, 0) /
                                googledata.length) *
                                10
                        ) / 10,
                    reviewavg:
                            ...
                            {!apiLoaded ? (
                                ...
                                <div className="kqGYQX bJFeVI">
                                    ...
                                    <div className="HeaderTotalReviews__Container-sc-1a7tbil-0 eaRlNB">
                                        {reviewobj.allReviewsSelected
                                            ? reviewobj.googlereviews.length + reviewobj.yelpreviews.length
                                            : reviewobj.googleSelected
