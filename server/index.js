        ...
        async function getGoogleData() {
          ...
          elements.forEach((element) => {
            ...
            const dataObject = {
              ...
              stars: element.querySelector('[role="img"][aria-label*="stars"]')
                .children.length,
        ...
        document.querySelectorAll("ul").forEach((el, index) => {
          ...
          if (el.children.length > 9 && el.textContent.includes("Tricia")) {
            ...
            for (let i = 0; i < el.children.length; i++) {
              ...
              const imgElements = element.querySelector(
...
app.get("/yelp", async (req, res) => {
  const data = await openPageAndScroll(
    "Yelp",
    "https://www.yelp.com/biz/prestigious-gaming-on-wheels-plus-queens"
  );
  // send json back to frontend
...
app.post("/bdaycard", async (req, res) => {
  const bdaycarddata = await openPageAndScroll(
    "BdayCard",
